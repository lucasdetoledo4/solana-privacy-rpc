...
async function main() {
  ...
  try {
    ...
    const directBalance = await directConnection.getBalance(
      new PublicKey(TEST_ADDRESSES[0]),
    );
    const directTime = Date.now() - directStart;

    console.log(
      `System Program balance: ${directBalance / LAMPORTS_PER_SOL} SOL`,
    );
    ...
    console.log(
      `- Current batch size: ${privateConnection.getPrivacyConfig().batchSize}`,
    );
  } catch (error) {
    ...
    console.log(
      "   Start it with: QUICKNODE_RPC_URL=http://localhost:8899 cargo run",
    );
  } finally {
    // Clean up
