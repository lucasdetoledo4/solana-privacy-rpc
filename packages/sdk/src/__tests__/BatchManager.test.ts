...
describe('BatchManager', () => {
    ...
    const createMockResponse = (
        transform: (queryId: string, index: number) => { success: boolean; data?: unknown; error?: string }
    ) => {
            ...
            expect(mockPost).toHaveBeenCalledWith(
                ...
                expect.objectContaining({
                    queries: expect.arrayContaining([
                        expect.objectContaining({ method: RpcMethod.GetBalance, pubkey: 'pubkey1' }),
                        expect.objectContaining({ method: RpcMethod.GetBalance, pubkey: 'pubkey2' }),
                        expect.objectContaining({ method: RpcMethod.GetBalance, pubkey: 'pubkey3' }),
                    ]),
                })
            );
